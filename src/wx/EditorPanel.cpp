#include "SSFEditorPanel.hpp"
#include "../resources/ResourceDocument.hpp"
...
namespace {
using neossf::ui::SSFEditorPanel;
...
enum : int {
    ...
    ID_FontDecrease,
    ID_FontReset,
    ID_Grid, ID_ModuleExit, ID_ModuleAbout
};

constexpr int kRecentFileBaseId = wxID_HIGHEST + 24000;
...
class NeoSSFPanelImpl final : public neossf::ui::SSFEditorPanel {
public:
    NeoSSFPanelImpl(wxWindow* parent, neomodules::Context context)
        : SSFEditorPanel(parent, std::move(context)) {
        buildMenus();
        buildMainWindow();
        ...
        if (!context_.embedded) tryLoadCachedTlk();
    ...
    bool openFile(const std::filesystem::path& path) override { openSsfPath(path); return true; }
    AppModel* activeModel() override { return hasActiveDocument() ? activeDocument().model.get() : nullptr; }
    void refreshActiveModel() override { if(hasActiveDocument()){markDocumentDirty();refreshAll();} }
    std::vector<std::filesystem::path> openPaths() const override {
        std::vector<std::filesystem::path> result;
        for(const auto& document:documents_) {
            if(!document.model->ssfFile().empty()) result.push_back(document.model->ssfFile());
        }
        return result;
    }
    bool openResource(neoshared::ResourceDocument input) override {
        if (activateResource(input.identity)) return true;
        auto loaded=neossf::ResourceDocument::load(std::move(input));
        ensureDocumentTabForOpen(); auto& document=activeDocument();
        *document.model=std::move(loaded.model);
        ...
        document.dirty=false; document.viewState.resetForNewDocument(); document.viewState.selectedLogicalRow=0;
        // Resolve preview text in the resource's game, never an unrelated cached installation.
        std::filesystem::path gameKey;
        for(const auto& source:document.protectedInputs) {
            auto extension=source.extension().string();
            std::transform(extension.begin(),extension.end(),extension.begin(),[](unsigned char c){return static_cast<char>(std::tolower(c));});
            if(extension==".key") {gameKey=source;break;}
        }
        if(!gameKey.empty())tryLoadResolvedTlkForPath(gameKey);
        if(context_.embedded && model().tlkLoaded() && !model().tlkFile().empty())
            document.protectedInputs.push_back(model().tlkFile());
        setFilterTerm({}); refreshAll(); grid_->SetFocus(); return true;
    }
    bool saveActiveAs(const std::filesystem::path& path) override {
        ...
        if (!saveModifiedTlkIfNeeded()) return false;
        model().saveSsf(path); markDocumentClean(); rememberRecentFile(path); refreshAll(); return true;
    }
...
private:
    void checkDestination(const std::filesystem::path& path) const {
        ...
        for(const auto& document:documents_) {
            ...
            const auto other=document.model->ssfFile();
        ...
        if(ext!=".ssf") throw std::runtime_error("Save as a .ssf file, not as a source archive.");
    }
    ...
    bool activeTabIsReusableForOpen() const {
        return hasActiveDocument() && documents_.size() == 1 && !tabDirty(activeDocument()) && model().ssfFile().empty() && activeDocument().resourceIdentity.empty();
    }
    ...
    bool confirmCloseDocumentTab(std::size_t index) {
        ...
        const int answer=wxMessageBox(wxui::toWx("Save changes to "+tabDisplayName(documents_[index])+"?"),
            "Close document",wxYES_NO|wxCANCEL|wxICON_QUESTION,this);
        if(answer==wxCANCEL)return false;
        if(answer==wxNO)return true;
        wxCommandEvent event;onSave(event);
        return !tabDirty(documents_[index]);
    }
    ...
    void buildMenus() {
        ...
        file->Append(ID_OpenSsf, "&Open SSF...\tCtrl+O");
        ...
        Bind(wxEVT_MENU, &NeoSSFPanelImpl::onLoadTlk, this, ID_LoadTlk);
        Bind(wxEVT_MENU, &NeoSSFPanelImpl::onNewSsf, this, ID_NewSsf);
        Bind(wxEVT_MENU, &NeoSSFPanelImpl::onOpenSsf, this, ID_OpenSsf);
        Bind(wxEVT_MENU, &NeoSSFPanelImpl::onOpenRecent, this, kRecentFileBaseId, kRecentFileBaseId + neosettings::kMaxRecentFiles - 1);
        Bind(wxEVT_MENU, &NeoSSFPanelImpl::onClearRecentFiles, this, kClearRecentFilesId);
        Bind(wxEVT_MENU, &NeoSSFPanelImpl::onSave, this, ID_Save);
        Bind(wxEVT_MENU, &NeoSSFPanelImpl::onSaveAs, this, ID_SaveAs);
        Bind(wxEVT_MENU, &NeoSSFPanelImpl::onCloseTab, this, ID_CloseTab);
        Bind(wxEVT_MENU, &NeoSSFPanelImpl::onCloseOtherTabs, this, ID_CloseOtherTabs);
        Bind(wxEVT_MENU, &NeoSSFPanelImpl::onNextTab, this, ID_NextTab);
        Bind(wxEVT_MENU, &NeoSSFPanelImpl::onPreviousTab, this, ID_PreviousTab);
        Bind(wxEVT_MENU, &NeoSSFPanelImpl::onModify, this, ID_Modify);
        Bind(wxEVT_MENU, &NeoSSFPanelImpl::onAddTlkEntry, this, ID_AddTlkEntry);
        Bind(wxEVT_MENU, &NeoSSFPanelImpl::onCopyCells, this, ID_CopyCells);
        Bind(wxEVT_MENU, &NeoSSFPanelImpl::onPasteCells, this, ID_PasteCells);
        Bind(wxEVT_MENU, &NeoSSFPanelImpl::onFilterPrompt, this, ID_Filter);
        Bind(wxEVT_MENU, &NeoSSFPanelImpl::onClearFilter, this, ID_ClearFilter);
        Bind(wxEVT_MENU, &NeoSSFPanelImpl::onFilterSelectedColumn, this, ID_FilterColumn);
        Bind(wxEVT_MENU, &NeoSSFPanelImpl::onClearSelectedColumnFilter, this, ID_ClearColumnFilter);
        Bind(wxEVT_MENU, &NeoSSFPanelImpl::onClearAllFilters, this, ID_ClearAllFilters);
        Bind(wxEVT_MENU, &NeoSSFPanelImpl::onMoveColumnLeft, this, ID_MoveColumnLeft);
        Bind(wxEVT_MENU, &NeoSSFPanelImpl::onMoveColumnRight, this, ID_MoveColumnRight);
        Bind(wxEVT_MENU, &NeoSSFPanelImpl::onResetColumnOrder, this, ID_ResetColumnOrder);
        Bind(wxEVT_MENU, &NeoSSFPanelImpl::onResetRowOrder, this, ID_ResetRowOrder);
        ...
        Bind(wxEVT_MENU, &NeoSSFPanelImpl::onExportPatcherPackage, this, ID_ExportPatcherPackage);
        Bind(wxEVT_MENU, &NeoSSFPanelImpl::onToggleDarkMode, this, ID_DarkMode);
        Bind(wxEVT_MENU, &NeoSSFPanelImpl::onIncreaseFontScale, this, ID_FontIncrease);
        Bind(wxEVT_MENU, &NeoSSFPanelImpl::onDecreaseFontScale, this, ID_FontDecrease);
        Bind(wxEVT_MENU, &NeoSSFPanelImpl::onResetFontScale, this, ID_FontReset);
    ...
    void openSsfPath(const std::filesystem::path& path) {
        if (path.empty()) return;
        for(std::size_t i=0;i<documents_.size();++i)
            if(neoshared::sameResourcePath(path,documents_[i].model->ssfFile())) { selectDocumentTab(i);return; }
        AppModel loaded; loaded.loadSsf(path);
        ensureDocumentTabForOpen();
        model()=std::move(loaded);
        activeDocument().resourceIdentity.clear(); activeDocument().protectedInputs.clear(); activeDocument().sourceDescription.clear();
        markDocumentClean();
    ...
    void buildMainWindow() {
        ...
        panel->SetSizer(root);
        auto* hostedLayout = new wxBoxSizer(wxVERTICAL);
        hostedLayout->Add(panel, 1, wxEXPAND);
        SetSizer(hostedLayout);

        Bind(wxEVT_BUTTON, &NeoSSFPanelImpl::dispatchButton, this, ID_LoadTlk);
        Bind(wxEVT_BUTTON, &NeoSSFPanelImpl::dispatchButton, this, ID_NewSsf);
        Bind(wxEVT_BUTTON, &NeoSSFPanelImpl::dispatchButton, this, ID_OpenSsf);
        Bind(wxEVT_BUTTON, &NeoSSFPanelImpl::dispatchButton, this, ID_Save);
        Bind(wxEVT_BUTTON, &NeoSSFPanelImpl::dispatchButton, this, ID_SaveAs);
        Bind(wxEVT_BUTTON, &NeoSSFPanelImpl::dispatchButton, this, ID_Modify);
        Bind(wxEVT_BUTTON, &NeoSSFPanelImpl::dispatchButton, this, ID_AddTlkEntry);
        if (filterText_) filterText_->Bind(wxEVT_TEXT, &NeoSSFPanelImpl::onFilterText, this);
        grid_->Bind(wxEVT_GRID_SELECT_CELL, &NeoSSFPanelImpl::onGridSelected, this);
        grid_->Bind(wxEVT_GRID_LABEL_LEFT_CLICK, &NeoSSFPanelImpl::onGridLabelClicked, this);
        grid_->Bind(wxEVT_GRID_LABEL_RIGHT_CLICK, &NeoSSFPanelImpl::onGridLabelRightClick, this);
        documentTabs_->Bind(wxEVT_AUINOTEBOOK_PAGE_CHANGED, &NeoSSFPanelImpl::onDocumentTabChanged, this);
        documentTabs_->Bind(wxEVT_AUINOTEBOOK_PAGE_CLOSE, &NeoSSFPanelImpl::onDocumentTabCloseRequested, this);
    }
    ...
    bool saveModifiedTlkIfNeeded() {
        ...
        std::filesystem::path target = context_.embedded ? std::filesystem::path{} : model().tlkFile();
        if (target.empty()) {
        ...
        validateHostOutput(target);
        neoshared::checkResourceOutput(target, activeDocument().protectedInputs);
    ...
    void refreshPaths() {
        ...
        ssfPath_->SetValue(wxui::toWx(model().ssfFile().empty() ? activeDocument().sourceDescription : pathText(model().ssfFile())));
    }
    ...
    void updateStatus() {
        updateActiveTabTitle();
        const std::string ssf = model().ssfFile().empty() ? activeDocument().untitledName : model().ssfFile().filename().string();
        ...
        setModuleStatusText( wxui::toWx(detail), 0);
        setModuleStatusText( wxui::toWx(tlk), 1);
    }
    ...
    void changeFontScaleSteps(int steps) {
        ...
        fontScale_ = next;
        if (!context_.embedded) settings_.setFontScale(fontScale_);
        applyFontScale();
    }
    ...
    void onExport(neotabular::Format format) {
        try {
            ...
            if (!file) return;
            validateHostOutput(*file);
            for(const auto& document:documents_) neoshared::checkResourceOutput(*file,document.protectedInputs);
            for(const auto& path:openPaths()) if(neoshared::sameResourcePath(*file,path))
                throw std::runtime_error("Export must not replace an open native document.");
    ...
    void onSave(wxCommandEvent& event) {
        try {
            ...
            if (model().ssfFile().empty()) {
                onSaveAs(event);
                return;
            }
            checkDestination(model().ssfFile());
    ...
    void onSaveAs(wxCommandEvent&) {
        try {
            ...
            const std::string defaultName = model().ssfFile().empty() ? activeDocument().untitledName : model().ssfFile().filename().string();
...
SSFEditorPanel* createEditorPanel(wxWindow* parent, neomodules::Context context) {
    return new NeoSSFPanelImpl(parent, std::move(context));
}
}
