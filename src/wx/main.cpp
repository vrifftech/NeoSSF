...
public:
    NeoSSFFrame():wxFrame(nullptr,wxID_ANY,wxui::toWx(std::string("NeoSSF v")+neossf::kVersion)) {
        wxIconBundle icons;
#if defined(__WXMSW__)
        wxIcon native("neossf",wxBITMAP_TYPE_ICO_RESOURCE);if(native.IsOk())icons.AddIcon(native);
#endif
        wxIcon fallback(neossf_icon_xpm);if(fallback.IsOk())icons.AddIcon(fallback);
        SetIcons(icons);
        neomodules::Context context;
        context.titleChanged=[this](const wxString& title){SetTitle(title);};
        context.closeRequested=[this]{Close();};
        panel_=neossf::ui::createEditorPanel(this,std::move(context));
    ...
    ~NeoSSFFrame() override {DestroyChildren();}
...
private:
    neossf::ui::SSFEditorPanel* panel_{};
    neosettings::AppSettings settings_{"NeoSSF"};
};
class NeoSSFApp final:public wxApp {
public:bool OnInit()override {
    SetAppName("NeoSSF");SetVendorName("Neo Tools");wxInitAllImageHandlers();
    auto* frame=new NeoSSFFrame;frame->Show();
